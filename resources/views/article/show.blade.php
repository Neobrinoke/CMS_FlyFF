...
@section('title', trans('title.article', ['title' => $article->title]))
...
@section('content')
    <div class="box">
        ...
        @if($article->authorized_comment)
            ...
            @auth
                <section class="ui fluid clearing segment">
                    <h3 class="ui dividing header">@lang('article.comment.post')</h3>
                    <form class="ui form {{ $errors->any() ? 'error' : '' }}" action="{{ route('article.comment.store', [$article]) }}" method="POST">
                        ...
                        <button class="ui right floated primary button" type="submit">@lang('article.comment.submit')</button>
                    </form>
                </section>
            @endauth
        @endif
